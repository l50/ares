# Ares ACL Agent Warp Gate Template

This template builds **Ares ACL Agent** images using Warp Gate. It supports
building **Docker images** (for `amd64` and `arm64`). The build provisions
Active Directory ACL exploitation tools using Ansible roles from the nimbus_range collection.
...
- `name`: Template name (`ares-acl-agent`)
...
warpgate build ares-acl-agent --only 'docker.*'
```
...
```bash
warpgate validate ares-acl-agent
```
