...
use ares_llm::tool_registry::AgentRole;
...
/// Normalize a role string to the canonical key used in `tools.yaml`.
///
/// `ARES_ROLE` (and therefore `WorkerConfig::worker_role`) can carry aliases
/// like `lateral_movement` whose `tools.yaml` key is `lateral`. Without this
/// step the build-script-generated `tools_for_role()` falls through to its
/// catch-all and returns an empty slice, which then ships an empty inventory
/// to Redis and trips the orchestrator preflight even when the binaries are
/// installed.
fn canonical_role(role: &str) -> &str {
    AgentRole::parse(role).map(|r| r.as_str()).unwrap_or(role)
}
...
pub async fn check_tools(role: &str) -> BTreeMap<String, bool> {
    let tools = tools_for_role(canonical_role(role));
...
mod tests {
    ...
    /// `ARES_ROLE` is set to `lateral_movement` in production but `tools.yaml`
    /// keys this role as `lateral`. `canonical_role` must bridge the two so
    /// the worker publishes a real inventory instead of an empty list.
    #[test]
    fn lateral_movement_alias_resolves_to_lateral_tools() {
        let direct = tools_for_role("lateral");
        let aliased = tools_for_role(canonical_role("lateral_movement"));
        assert_eq!(direct, aliased);
        assert!(aliased.contains(&"impacket-psexec"));
        assert!(aliased.contains(&"impacket-smbexec"));
        assert!(aliased.contains(&"impacket-secretsdump"));
    }
