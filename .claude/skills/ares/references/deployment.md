...
7. **`remote:sync` / `remote:sync:full` no longer exist — removed 2026-08-08.** They operated on `src/ares/**`, the pre-Rust Python tree, which this repo has not had for a long time; both printed per-pod sync failures and exited 0. `.claude/CLAUDE.md` used to prescribe `remote:sync:full TEAM=blue` for blue and now prescribes `task -y remote:rust:build && task -y remote:rust:deploy TEAM=blue`, which installs the freshly built binary on the blue pods without touching red or Redis. `task k8s:deploy TEAM=blue` is the heavier alternative (it also rolls the pods and re-patches the *red* orchestrator wrapper).
